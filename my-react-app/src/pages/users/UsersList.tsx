                ...
                <div className="bg-white dark:bg-neutral-900 rounded-2xl shadow-sm ring-1 ring-black/5 overflow-hidden">
                    ...
                    <div className="overflow-x-auto">
                        <table className="min-w-full text-left align-middle">
                            ...
                            {users.map((u) => (
                                <tr key={u.id} className="hover:bg-neutral-50/80 dark:hover:bg-white/5">
                                    <td className="px-5 py-4 text-sm text-neutral-600 dark:text-neutral-300 whitespace-nowrap">{u.id}</td>
                                    <td className="px-5 py-4">
                                        <div className="flex items-center gap-3">
                                            <div className="h-10 w-10 rounded-full bg-neutral-200 dark:bg-neutral-700 overflow-hidden grid place-items-center text-sm font-medium">
                                                {u.image ? (
...
// graceful fallback to initials if image fails
                                            ...
                                            <div>
                                                ...
                                                <div className="text-xs text-neutral-500">#{String(u.id).padStart(4, "0")}</div>
                                            </div>
                                        </div>
                                    </td>

                                    <td className="px-5 py-4 text-sm text-neutral-700 dark:text-neutral-200">{u.email}</td>
                ...
                <div className="md:hidden mt-4 space-y-3">
                    {users.map((u) => (
                        <div key={u.id} className="bg-white dark:bg-neutral-900 rounded-2xl shadow-sm ring-1 ring-black/5 p-4">
                            <div className="flex items-center gap-3">
                                <div className="h-10 w-10 rounded-full bg-neutral-200 dark:bg-neutral-700 overflow-hidden grid place-items-center text-sm font-medium">
                                    {u.image ? (
                                        <img
